...
public sealed class TcpBridgeServer(BridgeRuntime runtime, HostedBridgeHost hostStatus, string address) : IHostedBridgeServer
{
    ...
    {
        var endpoint = ResolveEndpoint(_address);
        _lifetime = CancellationTokenSource.CreateLinkedTokenSource(cancellationToken);
        _listener = new TcpListener(endpoint);
        _listener.Start();
        _acceptLoop = Task.Run(() => AcceptLoopAsync(_listener, _lifetime.Token), CancellationToken.None);

        var message = $"Live tcp bridge host listening at {endpoint}.";
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        _hostStatus.MarkStopped("Live tcp bridge host stopped.");
        HostedBridgeServerFactory.LogTransportMessage(
            _runtime,
            BridgeLogLevel.Info,
            "Live tcp bridge host stopped.");
    }
    ...
    private async Task AcceptLoopAsync(TcpListener listener, CancellationToken cancellationToken)
    {
        ...
        {
            _hostStatus.MarkStopped($"Live tcp bridge host failed: {ex.Message}");
            HostedBridgeServerFactory.LogTransportMessage(
                _runtime,
                BridgeLogLevel.Error,
                $"Live tcp bridge host failed: {ex}");
        }
    }
    ...
    {
        var diagnostics = StandaloneBridgeRpcDiagnostics.Start(_runtime, "tcp", request);
        try
        {
    ...
    private static IPEndPoint ResolveEndpoint(string address)
    {
        if (!IPEndPoint.TryParse(address, out var endpoint))
        {
            throw new InvalidOperationException(
                $"TCP bridge endpoint '{address}' is not a valid loopback host:port pair.");
        }

        if (!IPAddress.IsLoopback(endpoint.Address))
        {
            throw new InvalidOperationException(
                $"TCP bridge endpoint '{address}' must use a loopback address.");
        }
