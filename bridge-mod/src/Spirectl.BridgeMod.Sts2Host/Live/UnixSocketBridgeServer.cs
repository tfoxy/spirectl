...
public sealed class UnixSocketBridgeServer(BridgeRuntime runtime, HostedBridgeHost hostStatus, string socketPath) : IHostedBridgeServer
{
    ...
    public async Task StartAsync(CancellationToken cancellationToken = default)
    {
        ...
        _listener = listener;
        _acceptLoop = Task.Run(() => AcceptLoopAsync(listener, _lifetime.Token), CancellationToken.None);
        var message = $"Live unix-socket bridge host listening at {_socketPath}.";
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        _hostStatus.MarkStopped("Live unix-socket bridge host stopped.");
        HostedBridgeServerFactory.LogTransportMessage(
            _runtime,
            BridgeLogLevel.Info,
            "Live unix-socket bridge host stopped.");
    }
    ...
    private async Task AcceptLoopAsync(Socket listener, CancellationToken cancellationToken)
    {
        ...
        {
            _hostStatus.MarkStopped($"Live unix-socket bridge host failed: {ex.Message}");
            HostedBridgeServerFactory.LogTransportMessage(
                _runtime,
                BridgeLogLevel.Error,
                $"Live unix-socket bridge host failed: {ex}");
        }
    }
    ...
    {
        var diagnostics = StandaloneBridgeRpcDiagnostics.Start(_runtime, "unix-socket", request);
        try
        {
