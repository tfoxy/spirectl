...
grep -Fq 'Version="[4.5.1]"' "$project" || {
  echo "GodotSharp must use the reviewed exact version" >&2
  exit 1
}
...
work_dir="$(mktemp -d "${TMPDIR:-/tmp}/spirectl-reference-sdk.XXXXXX")"
trap 'rm -rf "$work_dir"' EXIT

dotnet restore "$project" --locked-mode -m:1
dotnet build "$project" --no-restore -m:1 \
  --configuration Release --output "$work_dir" \
...
printf '%s\n' \
  ...
  Spirectl.Sts2.ReferenceSdk.dll \
  Steamworks.NET.dll \
