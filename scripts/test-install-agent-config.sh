...
#
# This repo links no sibling-repo skills (EXTRA_SKILLS is empty) and its main checkout has no
# /.mcp.json, so those two code paths are driven from fixtures: a patched COPY of the installer with
# a fake EXTRA_SKILLS list, and a synthetic /.mcp.json written into the throwaway worktree.
#
...
# Pre-existing settings that must survive the jq merges. (A fresh worktree has no .claude/ at all.)
mkdir -p "$WT/.claude"
...
# EXTRA_SKILLS is empty in this repo, so nothing beyond skills/ may appear. (The sibling-link code
# path itself is exercised further down, with a patched copy of the installer.)
eq "no sibling skills linked when EXTRA_SKILLS is empty" \
...
if [ -f "$MAIN/.mcp.json" ]; then
  ...
  ok "…and the installer says there was nothing to link" grep -q 'nothing to link' "$INSTALL_LOG"
fi
...
echo "== sibling skills (fake EXTRA_SKILLS list) =="
# This repo links no sibling skills today, so the shared linking code is driven with a patched COPY
# of the installer: one sibling that is checked out, one that is not, and a locally installed REAL
# directory that must never be replaced by a symlink.
mkdir -p "$TMP/fake-sibling/skills/fake-skill"
printf -- '---\nname: fake-skill\ndescription: stub\n---\n' > "$TMP/fake-sibling/skills/fake-skill/SKILL.md"
mkdir -p "$WT/.claude/skills/fake-skill"
printf 'local\n' > "$WT/.claude/skills/fake-skill/MARKER"
sed 's|^EXTRA_SKILLS=()$|EXTRA_SKILLS=(../fake-sibling/skills/fake-skill ../missing-sibling/skills/missing-skill)|' \
  "$WT/scripts/install-agent-config.sh" > "$WT/scripts/install-agent-config.fake.sh"
grep -q 'fake-sibling' "$WT/scripts/install-agent-config.fake.sh" || die "EXTRA_SKILLS patch did not apply"
env HOME="$FAKEHOME_RO" bash "$WT/scripts/install-agent-config.fake.sh" > "$TMP/install-sibling.log" 2>&1
eq "patched install exits 0" 0 "$?"
ok "sibling skill .agents/skills/fake-skill is a symlink" test -L "$WT/.agents/skills/fake-skill"
ok "sibling skill .agents/skills/fake-skill resolves"     test -e "$WT/.agents/skills/fake-skill/SKILL.md"
ok "a real skill dir is not replaced"                     test -f "$WT/.claude/skills/fake-skill/MARKER"
ok "a real skill dir stays a real dir"                    test ! -L "$WT/.claude/skills/fake-skill"
ok "a sibling that is not checked out is skipped"         grep -q 'skipped missing-skill' "$TMP/install-sibling.log"
ok "…and nothing is linked for it"                        test ! -e "$WT/.agents/skills/missing-skill"
