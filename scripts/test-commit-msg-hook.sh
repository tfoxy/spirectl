...
accepts "deep scope"                  'test(bridge/producers): cover the encounter fixture routes'
accepts "subject of exactly 72" "$(printf 'feat: %066d\n\nbody\n\nChangelog: yes' 1)"
accepts "feat with a changelog line" 'feat(cli): add --json to game status

Emit the same fields as the human output.so a card can be removed by touch
alone, matching the mouse path.

Changelog: `sts2 game status --json` prints the same fields as the human output.'
accepts "fix with Changelog: none"   'fix(validation): scan tracked blobs reliably

Changelog: none'
accepts "perf with a trailer"        'perf(bridge): reuse the producer buffer across frames

Changelog: Halves allocation churn in long capture sessions.'
accepts "a breaking change"          'feat(proto)!: drop the v0 frame reader

BREAKING CHANGE: consumers pinned to frame version 0 must regenerate.

Changelog: Regenerate protobuf bindings after updating.'
accepts "the release commit"         'chore(release): v0.1.2'
accepts "a merge commit"             "Merge branch 'cc-r21-b5-gradient'"
accepts "a revert"                   'Revert "feat(cli): add --json to game status"'
accepts "a fixup"                    'fixup! feat(cli): add --json to game status'
...
```
cargo test -p spirectl-cli -- --exact "a very long test name that runs well past one hundred columns"
```'
...
rejects "a retired prefix"      'cleanup(bridge): retire the legacy reflection path' \
        'cleanup -> refactor'
...
$(printf 'word %.0s' $(seq 1 25))" \
        'limit is 100'
rejects "feat without a trailer" 'feat(cli): add --json to game status

Emit the same fields as the human output.' \
        'Changelog: none'
