...
- Before live smoke reads against an existing game, run `cargo run -p sts2 -- --json game bridge-health`; it distinguishes missing, refused/stale, timed-out, version-mismatched, presentation/render RPC-incompatible, and reachable/current bridge states without deploying, restarting, killing, or mutating game files. Add `--verbose` when the full diagnostic payload is needed.
