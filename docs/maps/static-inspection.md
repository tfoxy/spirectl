...
- `code verify-references` also sees only the bindings the compiler emitted. The bridge's by-name reflection reads carry no `MemberRef`, so they are swept separately: `scripts/verify-reflected-game-members.sh <assemblies-dir> <assemblies-dir>` reports the member-name literals that resolve in one game build and not the other, and the lane seam it excludes is gated at startup by `GameApiManifest` instead.
